...
public class RemoveDuplicates {
    ...
    public static void removeDuplicates(List<Integer> nums) {
        if(nums == null || nums.size() == 0) {
            System.out.println("No list has been found");
        }
    ...
    /*static List<Integer> insertionSorting(List<Integer> arr) {
        ...
        for(int i = 0; i<arr.size(); i++) {
            ...
            while(i > 0 && arr.get(i-1) > currentElement) {
                arr.get(i) = arr.get(i-1);                    // arr.get(i) should be a variable. How to make it so within ArrayList scope?
                index--;
            }
            arr.get(i) = currentElement;
        }
        return arr;
    */
}
